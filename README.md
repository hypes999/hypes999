### Olá, eu sou o hypes 👋

Estudante de **Cibersegurança**  
Focado em **Blue Team** e **Automação de Segurança**  
Atualmente a trabalhar em laboratórios, projetos e desafios práticos.

---

### TryHackMe & HackTheBox

- TryHackMe: [hypes999](https://tryhackme.com/p/hypes999)  
- HackTheBox: [hypes999](https://app.hackthebox.com/profile/2660530)

---

### 🚀 Projetos em Destaque

- **[Cybersecurity Toolkit](https://github.com/hypes999/cybersecurity-toolkit)** — Coleção de ferramentas em Python (Diceware PT‑PT, futuros scanners, etc.).  
- **[Cyber Lab](https://github.com/hypes999/cyber-lab)** — Projetos práticos, CTFs e anotações do curso.  
- **[Homelab](https://github.com/hypes999/Homelab)** — Homelab Proxmox + Nextcloud + outros serviços.

---

### 🛠️ Tecnologias & Interesses

`Linux` • `Blue Team` • `Networking` • `Pentesting` • `Python` • `Automação`

<p align="center">
  <img src="https://skillicons.dev/icons?i=bash,linux,kali,py,js,cloudflare,ps,powershell,vscode,windows" />
</p>
